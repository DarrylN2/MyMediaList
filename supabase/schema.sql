...
create or replace function public.set_user_media_updated_at()
returns trigger as $$
begin
  new.updated_at = now();

  if new.first_rated_at is null
     and new.user_rating is not null
     and (tg_op = 'INSERT' or old.user_rating is null) then
    new.first_rated_at = now();
  end if;
...
for each row
execute procedure public.set_user_media_updated_at();
